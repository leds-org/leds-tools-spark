...
import TelefoneService from '../../../services/requires/TelefoneRequires';

import dayjs from 'dayjs';


const { list, post, getById, update } = TelefoneService();
...
const onSubmit = async () => {
    try {
        ...
        Swal.fire({
            ...
            router.push({ path: '/Telefone/IndexTelefone' });
        }
        );
...
const page = ref({ title: 'Cadastrar Telefone' });
const submitButtonText = ref('Cadastrar Telefone');

onMounted(async () => {
    if (params.id) {
        await getPost(params.id);
        page.value.title = 'Editar Telefone';
        breadcrumbs.value[1].text = page.value.title;
        submitButtonText.value = 'Editar Telefone';
    }
